...
path.src.style[1] = "!" + path.src.srcPath + path.src.components + "/vendor.scss";
...
function script() {
    let pipeline = gulp.src(path.src.script)
        .pipe(plumber());

    if (isProd()) {
        pipeline = pipeline.pipe(webpackStream(webpackConf, webpack))
    ...
    return pipeline.pipe(gulp.dest(path.dist.script))
        .pipe(browserSync.reload({ stream: true }));
}


function vendorScripts() {
    return gulp.src(libFilesJs)
...
function ttf2woff2Converter() {
    return gulp.src(path.src.font[0].slice(0, -6) + "app/*.ttf")
        .pipe(ttf2woff2())
 ...
 */
function otf2ttf() {
    return gulp.src(path.src.font[0].slice(0, -6) + "app/*")
        .pipe(fonter({
            formats: ['ttf']
        }))
